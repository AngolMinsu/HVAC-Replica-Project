...
HuCommand parseHuCommand(String line) {
  ...
  if (command == "read") {
    ...
    if (arg == "psgtemp" || arg == "ptemp") return makeRead(CAN_SIGNAL_PASSENGER_TEMPERATURE);
    ...
    if (arg == "mute") return makeRead(CAN_SIGNAL_MUTE);
    if (arg == "home") return makeRead(CAN_SIGNAL_HOME);
    if (arg == "mediamode") return makeRead(CAN_SIGNAL_MEDIA_MODE);
    if (arg == "mediaindex" || arg == "track") return makeRead(CAN_SIGNAL_MEDIA_INDEX);
    return invalidCommand();
  }
  ...
  if (command == "psgtemp" || command == "ptemp") {
    uint8_t value = 0;
    if (!parseDecimalByte(arg, GDS_TEMP_MIN, GDS_TEMP_MAX, value)) return invalidCommand();
    return makeWrite(CAN_SIGNAL_PASSENGER_TEMPERATURE, value);
  }
  ...
  if (command == "wind") {
    ...
    if (arg == "off") return makeWrite(CAN_SIGNAL_MODE, 4);
    return invalidCommand();
  }

  if (command == "mediaindex" || command == "track") {
    uint8_t value = 0;
    if (!parseDecimalByte(arg, 0, GDS_MEDIA_INDEX_MAX, value)) return invalidCommand();
    return makeWrite(CAN_SIGNAL_MEDIA_INDEX, value);
  }
  ...
  if (command == "mute" && parseOnOffValue(arg, value)) return makeWrite(CAN_SIGNAL_MUTE, value);
  if (command == "home" && parseOnOffValue(arg, value)) return makeWrite(CAN_SIGNAL_HOME, value);
  if (command == "mediamode" && parseOnOffValue(arg, value)) return makeWrite(CAN_SIGNAL_MEDIA_MODE, value);
...
void printHuHelp() {
  ...
  Serial.println("  psgtemp 18..30");
  Serial.println("  volume 0..30");
  Serial.println("  mediaindex 0..30");
  Serial.println("  track 0..30");
  Serial.println("  screen datc|info");
  Serial.println("  wind face|foot|def|mix|off");
  ...
  Serial.println("  home on|off");
  Serial.println("  mediamode on|off");
  Serial.println("  read fan|temp|psgtemp|screen|media|volume|map|mute|home|mediamode|mediaindex|track");
  Serial.println();
}
