...
#define CAN_PORT         GDS_PIN_CAN_CS
...
uint8_t readButtonEvent() {
  ...
  return detectButtonEvent(buttonHistory, levels, millis(), GDS_DEBOUNCE_DELAY_MS);
}
...
uint8_t readEncoderEvent() {
  ...
  return detectEncoderEvent(encoderHistory, levels, millis(), GDS_ENCODER_DEBOUNCE_DELAY_MS);
}
