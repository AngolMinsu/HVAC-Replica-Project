...
static uint8_t driverDirectionFromB(uint8_t bLevel) {
  return (bLevel == HIGH) ? ENCODER_EVENT_DRIVER_CW : ENCODER_EVENT_DRIVER_CCW;
}

static uint8_t passengerDirectionFromB(uint8_t bLevel) {
  return (bLevel == HIGH) ? ENCODER_EVENT_PASSENGER_CW : ENCODER_EVENT_PASSENGER_CCW;
}
...
uint8_t detectEncoderEvent(EncoderHistory& history, const EncoderLevels& levels, unsigned long now, unsigned long debounceDelay) {
  ...
  if (now - history.lastEventTime > debounceDelay) {
    if (isEncoderFallingEdge(history.prevDriverSw, levels.driverSw)) {
      ...
      event = driverDirectionFromB(levels.driverB);
    } else if (isEncoderFallingEdge(history.prevPassengerA, levels.passengerA)) {
      event = passengerDirectionFromB(levels.passengerB);
    }
