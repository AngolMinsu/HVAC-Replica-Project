#line 1 "E:\\030_Portfolio\\010_Arduino\\040_CAN\\HVAC Replica\\main\\MKBD_Module_Structure.md"
# MKBD 모듈 분리 구조

## 1. 분리 목적

MKBD 코드는 처음에는 `main.ino` 중심으로 버튼 입력, 상태 변경, OLED 출력, 팬 제어, CAN 송수신이 함께 들어갈 수 있었다.
...
`main.ino`는 세부 로직을 직접 처리하지 않고, 각 모듈의 함수를 호출하는 역할에 집중한다.
...
`main.ino`가 읽은 버튼 값을 전달받아 이벤트를 계산한다.
...
이 프로젝트는 `main.ino`에 모든 코드를 넣지 않고, 입력 처리, 상태 변경, 화면 출력, CAN 송수신, Payload 해석을 모듈별로 분리했다.
